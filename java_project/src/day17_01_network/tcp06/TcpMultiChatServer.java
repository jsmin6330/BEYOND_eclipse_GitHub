...
public class TcpMultiChatServer { //outer class
	...
	public TcpMultiChatServer() { //constructor method
		...
		Collections.synchronizedMap(clients); //동기화 시킨다.
	}
	
	public void start() {
		ServerSocket ss = null; //server socket1
		Socket s = null; //client socket2
		
		try {
			ss = new ServerSocket(7777);
			System.out.println("서버 시작되었습니다. 채팅 준비 완료!!");
			while(true) {
				s = ss.accept(); //응답대기, 클라이언트 소켓
				//클라이언트가 접속하면 아래 코드 실행
	...
	public void sendToAll(String msg) {
		...
		while(it.hasNext()) {
			try {
				DataOutputStream dos = (DataOutputStream)clients.get(it.next()); //해당 키 값으로 value
	...
	class ServerReceiver extends Thread{ //Inner Class
		...
		public void run() { //스레드 실행부(구현부)
			...
			try {
				...
				clients.put(name, dos); //HashMap에 데이터 넣기
