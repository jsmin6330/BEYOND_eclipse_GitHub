package day18_02_mvc.controller;
...
import day18_02_mvc.connUtill.CloseHelper;
import day18_02_mvc.connUtill.ConnectionHelper;
import day18_02_mvc.model.GiftDTO;

public class GiftController {//모델과 VIEW의 중개역할
	//연결, 삽입, 삭제, 수정, 검색.....
		...
		while( true ) {
			...
			switch ( sc.nextInt() ) {
				...
				case 1:
					...
					selectAll(dto.getTableName());
					break;
				case 2:	selectAll(dto.getTableName());
					update(dto.getTableName());
					selectAll(dto.getTableName());
					break;
				case 3:	selectAll(dto.getTableName());
					break;
				case 4:	selectByGno(dto.getTableName());
					break;
				case 5:	
							selectAll(dto.getTableName());
							delete(dto.getTableName());
							break;
				case 6:	close();
	...
	public static void update(String className) throws SQLException {  //
		...
		while(true) {
			...
			b:
			while(true) {
				System.out.println("\n\n1.GNO\t2.GNAME\t3.G_START\t4.G_END\t5.price\t0.exit");
				...
				switch (choice) {
					...
					case 5 : System.out.print("바뀔 price : ");
						stmt.execute("update " + className + " set price = " + sc.nextInt() + " where gno = " + gno);
						break b;  // in while break;
	...
	public static void insert() {
		...
		System.out.print("price : ");		String price = sc.next();
		
		try {
			pstmt = conn.prepareStatement("INSERT INTO GIFT_SET VALUES( ?, ?, ?, ?, ? )");
			pstmt.setString(1, gno);
	...
	public static void selectAll(String className) throws SQLException {
		...
		ResultSetMetaData rsmd = rs.getMetaData(); //ResultSetMetaData: 인터페이스
		//ResultSetMetaData는 정보들을 가지고 있다.
		int count = rsmd.getColumnCount();
